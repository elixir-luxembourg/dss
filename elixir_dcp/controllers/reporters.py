...
def generate_submission_pdf(sub_id):
    ...
    bootstrap_css = CSS(filename=app.static_folder + '/vendor/node_modules/bootstrap/dist/css/bootstrap.css')
    page_css = CSS(string='''
        @page {
            size: A4;
            margin: 1cm;
    ...
    pdf_bytes = html.write_pdf(
        stylesheets=[bootstrap_css, page_css],
        font_config=font_config
    )
    
    response = make_response(pdf_bytes)
    response.headers['Content-Type'] = 'application/pdf'
    response.headers['Content-Disposition'] = f'inline; filename=submission_{submission_rec.ref_name}.pdf'
    return response
...
def generate_submission_docx(sub_id):
    ...
    doc.save(doc_io)
    doc_io.seek(0)
    return send_file(doc_io, mimetype=".docx", as_attachment=True, download_name=f'Submission_{sub.ref_name}.docx')

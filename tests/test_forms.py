...
class FormValidatorsTest(BaseTest):
    ...
    def test_signup_form(self):
        f1 = SignupForm(elixir_sub_id="DUMMY ELIXIR SUB ID NOT VALIDATED",
                         first_name="Pinar",
                         last_name="á è í ö â ñ ç ř α Š ", phone_no="125736 87--34 ",  institution ="LcsB -(LCSB)", email="pinar.alper@uni.lu")

        self.assertTrue(f1.validate())

        f2 = SignupForm(elixir_sub_id="DUMMY ELIXIR SUB ID NOT VALIDATED",
                    first_name="Pinar1 23 ",
                    last_name="è í ö â ñ ç ř α Š", phone_no="125736 87--34 ",  institution ="LcsB -DROP TABLE ..;(LCSB)", email="pinar.alper@uni.lu")
        self.assertFalse(f2.validate())
        print(f2.errors)

        f3 = SignupForm(elixir_sub_id="DUMMY ELIXIR SUB ID NOT VALIDATED",
                        first_name="Pinar",
                        last_name="Alper958945", phone_no="125736 87<tags></tags>34 ",  institution ="LcsB -(LCSB)", email="pinar.alper@uni.lu")

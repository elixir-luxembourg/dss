...
from elixir_dcp.models.submission import Submission, SubmissionStatusEnum, SubmissionScope, SubmissionAccess, \
    SubmissionStudyDish, SubmissionAttachment
from elixir_dcp.models.services import register_new_user, assign_role_to_user, create_sub, steer_sub, \
    update_submission_basic_info, revert_sub, deactivate_user, delete_sub, export_submission, export_studies, export_dishes
...
class ModelPersistenceTest(BaseTest):
    ...
    def test_export_submission(self):
        ...
        dish_rec.submission_id = submission_rec.id
        dish_rec.study_name = ''
        dish_rec.study_description = ''
        dish_rec.study_types_json = ''
        dish_rec.estimate_data_size_code = 'm'
        dish_rec.data_types_json = json.dumps(["Genomics_variant_array","RNASeq"])
        dish_rec.subjects_minors = True
        dish_rec.subjects_notes = 'mothers and babies'
        dish_rec.consent_notes = 'Consent is consistent among all subjects'
        ...
        dish_rec.study_description = ''
        dish_rec.study_types_json = ''
        dish_rec.estimate_data_size_code = 'l'
        dish_rec.data_types_json = json.dumps(["Transcriptome_array","RNASeq"])
        dish_rec.consent_status_code = 't'
        dish_rec.consent_notes = 'There are three primary consent groups'

...
class Config(object):
     ...
     APP_MENU_BAR_ITEMS = [
         ('Admin', None, [('Submissions', 'list_submissions'), ('Users', 'list_users'), ('Email Notifications', 'list_notifications')], 'admin'),
         ('My Submissions', 'list_my_submissions', [], 'data_provider'),
     ]
     DATA_INIT = START_UP_DATA

     # Configuration from environment variables
     DATA_STEWARDS_MAILS = os.environ.get('DATA_STEWARDS_MAILS').split(',') if os.environ.get('DATA_STEWARDS_MAILS') else []
     ...
     AUTHENTICATION_DICT = {'steward1@uni.lu': 'steward1', 'submitter1@some.edu': 'submitter1', 'submitter2@some.edu': 'submitter2'}

     # OIDC config
     OIDC_SCOPES = os.environ.get('OIDC_SCOPE').split() if os.environ.get('OIDC_SCOPE') else ['openid', 'email', 'profile']
     ...
     PERMANENT_SESSION_LIFETIME = int(os.environ.get('PERMANENT_SESSION_LIFETIME')) if os.environ.get('PERMANENT_SESSION_LIFETIME') else 3600
